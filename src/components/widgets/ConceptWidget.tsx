...
export default function ConceptWidget() {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div className="p-3 bg-slate-50 rounded-lg">
            <span className="block text-xs text-slate-500 uppercase tracking-wider mb-1">Ubicación</span>
            <p className="font-medium text-slate-900">
              {concept.location?.city || 'Ciudad'}, {concept.location?.country || 'País'}
            </p>
            {concept.location?.hasLocation && (
               <p className="text-xs text-green-600 mt-1 flex items-center gap-1">
                 <CheckCircle2 className="w-3 h-3" /> Local asegurado ({concept.location.sizeSqM}m²)
               </p>
            )}
          </div>
          <div className="p-3 bg-slate-50 rounded-lg">
            <span className="block text-xs text-slate-500 uppercase tracking-wider mb-1">Estilo</span>
            <p className="font-medium text-slate-900">
              {concept.style?.cuisine || 'Cocina'} - {concept.style?.atmosphere || 'Ambiente'}
            </p>
          </div>
          <div className="p-3 bg-slate-50 rounded-lg">
            <span className="block text-xs text-slate-500 uppercase tracking-wider mb-1">Ticket Zona (Est.)</span>
            <p className="font-medium text-slate-900">
              {concept.location?.areaAverageTicket ? `${concept.location.areaAverageTicket}€` : '-'}
            </p>
          </div>
          <div className="p-3 bg-slate-50 rounded-lg">
            <span className="block text-xs text-slate-500 uppercase tracking-wider mb-1">Equipo</span>
            <p className="font-medium text-slate-900">
              {concept.team?.kitchenStaff || 0} Cocina / {concept.team?.serviceStaff || 0} Sala
            </p>
            {concept.team?.notes && (
              <p className="text-xs text-slate-500 mt-1 italic line-clamp-2">{concept.team.notes}</p>
            )}
          </div>
        </div>
      </div>
